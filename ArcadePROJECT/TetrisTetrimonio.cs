...
namespace ArcadePROJECT {
    class Tetrimino {
            ...
            for (int i = 0; i < figura.GetLength(0); i++) {
                for (int j = 0; j < figura.GetLength(1); j++) {
                    if (figura[i, j] == 1) {
                        ...
                        Console.Write(ProgramTetris.cuadrado);
                    }
                }
            }
        }
        ...
        public void Soltar() {
            ...
            if (HayAlgoDebajo()) {
                for (int i = 0; i < 4; i++) {
                    ProgramTetris.posicionCaidaPiezaEnGrid[posicion[i][0], posicion[i][1]] = 1;
                }
                ProgramTetris.dejarCaer = true;
        ...
        public bool HayAlgoDebajo() {
            for (int i = 0; i < 4; i++) {
                if (posicion[i][0] + 1 >= 23)
                    return true;
                if (posicion[i][0] + 1 < 23) {
                    if (ProgramTetris.posicionCaidaPiezaEnGrid[posicion[i][0] + 1, posicion[i][1]] == 1) {
                        return true;
                    }
                }
            }
            ...
            for (int i = 0; i < 4; i++) {
                if (coordenadas.Max() - coordenadas.Min() == 3) {
                    if (coordenadas.Max() == location[i][0] | coordenadas.Max() - 1 == location[i][0]) {
                        if (ProgramTetris.posicionCaidaPiezaEnGrid[location[i][0], location[i][1]] == 1) {
                            return true;
                        }
                    }
        ...
        public bool HayAlgoIzquierda() {
            for (int i = 0; i < 4; i++) {
                if (posicion[i][1] == 0) {
                    return true;
                } else if (ProgramTetris.posicionCaidaPiezaEnGrid[posicion[i][0], posicion[i][1] - 1] == 1) {
                    return true;
                }
            }
            return false;
        ...
        public void Actualizar() {
            for (int i = 0; i < 23; i++) {
                for (int j = 0; j < 10; j++) {
                    ProgramTetris.areaJuego[i, j] = 0;
                }
            }
            for (int i = 0; i < 4; i++) {
                ProgramTetris.areaJuego[posicion[i][0], posicion[i][1]] = 1;
            }
            ProgramTetris.Dibujar();
        }
